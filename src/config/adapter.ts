...
export class TypeOrmDbAdapter<T> {
  ...
  public createCursor(params: any, isCounting: boolean = false) {
    if (params) {
      let query: FindManyOptions<T> = !params?.query?.where
        ? {
            where: params.query || {},
          }
        : params.query;
      if (params.searchFields) {
        query.select = params.searchFields;
      }
  ...
  private _enrichWithOptionalParameters(
    ...
    if (params.search) {
      throw new Error(
        "Not supported because of missing or clause meanwhile in typeorm"
      );
    }
